...
class GetZimbra():
    def __init__(self, hostname, user, password, account):
    ...
    def modify_signature(self,func):
        """
        ...
        self.request.add_request(
            ...
            {
                "id": accountid, 'a': [
                    {'n': self.attr, '_content': self.signature}
                ]                
            },
            'urn:zimbraAdmin'
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--account', action='store', dest='account',
                        help='account to get accounts info\
                        \n\tExample: \n--account test@example.com', required=True)
    ...
    zmteste = GetZimbra(argslist.hostname, argslist.admin, argslist.password, argslist.account)
    print(zmteste.connect())
    print(zmteste.modify_signature(zmteste.get_accountinf()))
